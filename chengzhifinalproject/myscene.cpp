...
else if(event->key() == Qt::Key_L)
{
  ...
  if (! enemies[index_enemy]->getDefeated()) {
      m->protHurt();checkProtNeedDie();
  if (std::count(PEnemyIndex.begin(), PEnemyIndex.end(), index_enemy)) { showPEnemyAttackSignal();}else{showEnemyAttackSignal(); }}
}}

else if (event->key() == Qt::Key_Left && !checkwall(getProtX()-1,getProtY())){
  userMoveView(); protpixmapItem ->moveBy(scalNum,0); m->setProtX(getProtX()+1);
  index_enemy=checkIsEnemy( getProtX(),getProtY());  if (index_enemy != -1) {
  if (! enemies[index_enemy]->getDefeated()) {
      m->protHurt();checkProtNeedDie();
  if (std::count(PEnemyIndex.begin(), PEnemyIndex.end(), index_enemy)) { showPEnemyAttackSignal();}else{showEnemyAttackSignal(); }}
...
else if (event->key() == Qt::Key_Right && checkwall(getProtX()+1,getProtY())) {// 向右移动
  userMoveView();protpixmapItem ->moveBy(scalNum, 0); m->setProtX(getProtX()+1);emit moveViewSignal();
  index_enemy=checkIsEnemy( getProtX(),getProtY()); if (index_enemy != -1) {
  if (! enemies[index_enemy]->getDefeated()) {
      m->protHurt();checkProtNeedDie();
...
else if (event->key() == Qt::Key_Right  && !checkwall(getProtX()+1,getProtY())){
  userMoveView(); protpixmapItem ->moveBy(-scalNum,0); m->setProtX(getProtX()-1);
  index_enemy=checkIsEnemy( getProtX(),getProtY()); if (index_enemy != -1) {
  if (! enemies[index_enemy]->getDefeated()) {
      m->protHurt();checkProtNeedDie();
...
else if (event->key() == Qt::Key_Up && checkwall(getProtX(),getProtY()-1)) {// 向上移动
   userMoveView(); protpixmapItem->moveBy(0, -scalNum);m->setProtY(getProtY()-1);emit moveViewSignal();
   index_enemy=checkIsEnemy( getProtX(),getProtY()); if (index_enemy != -1) {
       if (! enemies[index_enemy]->getDefeated()) {
              m->protHurt();checkProtNeedDie();
   if (std::count(PEnemyIndex.begin(), PEnemyIndex.end(), index_enemy)) { showPEnemyAttackSignal();}else{showEnemyAttackSignal(); }}
  ...
  userMoveView();  protpixmapItem->moveBy(0, +scalNum);m->setProtY(getProtY()+1);
  index_enemy=checkIsEnemy( getProtX(),getProtY()); if (index_enemy != -1) {
   if (! enemies[index_enemy]->getDefeated()) {
     m->protHurt();checkProtNeedDie();
  if (std::count(PEnemyIndex.begin(), PEnemyIndex.end(), index_enemy)) {showPEnemyAttackSignal();}else{showEnemyAttackSignal(); }}
...
else if (event->key() == Qt::Key_Down && checkwall(getProtX(),getProtY()+1)){ // 向下移动
  userMoveView();  protpixmapItem->moveBy(0, scalNum);m->setProtY(getProtY()+1);emit moveViewSignal();
  index_enemy=checkIsEnemy( getProtX(),getProtY()); if (index_enemy != -1) {
   if (! enemies[index_enemy]->getDefeated()) {
      m->protHurt();checkProtNeedDie();
